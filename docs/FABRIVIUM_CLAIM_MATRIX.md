...
|---|---|---|---|---|
| CEC-120 goes from 1,435 to 1,900 units/day | **DEMO-VERIFIED** | `docs/FABRIVIUM_CEC_CASE_PROVENANCE.md`, `examples/electronics/CEC-120_competition_case.json`, `backend/scripts/golden_journey_run.py` | "A verified case study: 1,435 â†’ 1,900/day" | "Fabrivium optimises any line to target" |
| The plan buys no new machines and costs â‚¬18,000/day more | **DEMO-VERIFIED** | Same run | "The chosen plan added a shift, not a machine" | "Fabrivium finds free capacity" |
...
| Bottleneck | Cable connection Ã—2 | Screw fastening Ã—6 |
| Simulations | **23** | 28 |
| Public? | **Yes** | No â€” internal regression evidence |
...
| Mechanical product runs the same pipeline to a verified simulation | **DEMO-VERIFIED** | `examples/generalization/results/case_m.json` â€” 420/420, demand met | "A mechanical actuator ran the same path to a verified 420/day" | "Fabrivium supports mechanical assembly" (7 of its operations have no rule) |
| Packaging product runs the same pipeline to a verified simulation | **DEMO-VERIFIED** | `case_p.json` â€” 2,952/4,000, bottleneck `m-packaging`, 3 strategy options | "A filling line ran the same path, found its constraint and generated priced-incomplete options" | "Fabrivium plans packaging lines" |
...
| Claim | Status | Evidence | Allowed wording | Forbidden |
|---|---|---|---|---|
| A language model cannot reach the simulation core | **TESTED** | `backend/tests/test_deterministic_core_isolation.py` â€” transitive import graph, 32 core modules | "Enforced by an architecture test, transitively" | "The AI cannot affect results" (it proposes values a person may accept) |
...
| Simulation is deterministic discrete-event | **IMPLEMENTED** | SimPy; `services/simulation.py` | "Deterministic discrete-event simulation" | "Stochastic/Monte-Carlo analysis" â€” there is none |
...
| Claim | Status | Evidence | Allowed wording | Forbidden |
|---|---|---|---|---|
| Several operations can share one physical resource | **TESTED** | `ConceptOperationGroup`; 27 tests in `backend/tests/test_operation_grouping.py` | "An engineer can declare that one cell performs several operations" | "Fabrivium designs cells" â€” it records the engineer's decision |
...
| Cell work content is the sum of its operations | **TESTED** | `backend/tests/test_operation_grouping.py::test_cell_work_content_is_the_sum_and_never_less` | "Sequential cell work content = the sum" | "Fabrivium models cell concurrency" â€” only SEQUENTIAL exists |
| Grouping is reversible and invalidates verification | **TESTED** | API tests + channel test | "Explicit, reversible, and it re-opens verification" | â€” |
...
| The process-family catalog reaches the model dynamically | **TESTED** | Same file; composed from `services.process_families` | "The model is given the canonical twelve families, generated not typed" | â€” |
...
| **A live Bob call has been made** | **NOT DONE** | No credential in this environment; `backend/scripts/bob_smoke.py` exists to prove it | "Provider implemented and contract-tested; live validation not run â€” no credential available" | "Integrated with IBM Bob", "Powered by IBM Bob" |
...
| Claim | Status | Evidence | Allowed wording | Forbidden |
|---|---|---|---|---|
| A real `.spp` model is produced and reopens correctly | **DEMO-VERIFIED** | `docs/SIEMENS_HANDOFF_VERIFICATION.md` â€” OLE header, topology and cycle times checked in Plant Simulation | "A real Siemens Plant Simulation handoff, verified in the application" | "Fabrivium runs inside Plant Simulation" |
...
| The two simulators agree | **NO** | `docs/CROSS_SIMULATOR_SEMANTICS.md` documents where they differ | "Where they differ, the difference is documented" | "Results match Siemens" |
...
## 8. Coverage â€” the limits that must travel with any generalization claim

| Fact | Status | Required qualifier |
|---|---|---|
| 12 process families | **IMPLEMENTED** | â€” |
...
2. **"Powered by IBM Bob."** No live call has been made from this
   repository. Say *"an IBM Bob provider is implemented and contract-tested;
   live validation is pending a credential."*
...
3. **"Verified by simulation."** True of throughput, and only of throughput,
   ...
   source document was completely understood â€” coverage measures extraction,
