...
| **DOCUMENTED** | Supported by repository documentation, not by an end-to-end run |
| **TRUE** | A factual state or limitation supported by the cited evidence |
| **NOT DONE / NO** | Explicitly absent; must not be presented as implemented or verified |
| **EXTERNALLY BLOCKED** | The integration exists, but live validation is blocked by an external account or service |
| **ROADMAP** | Not built. May be described as intended, never as present |

Qualifiers such as *development-time* or *and now said on screen* narrow a
base status; they do not make the claim stronger.
...
| The competition case is one of two CEC configurations | **DOCUMENTED** | `docs/FABRIVIUM_CEC_CASE_PROVENANCE.md` | "Public material quotes the competition case only" | Calling the unattended baseline "the canonical CEC case" |
...
| Packaging product runs the same pipeline to a verified simulation | **DEMO-VERIFIED** | `examples/generalization/results/case_p.json` — 2,952/4,000, bottleneck `m-packaging`, 3 strategy options | "A filling line ran the same path, found its constraint and generated priced-incomplete options" | "Fabrivium plans packaging lines" |
| Seven non-CEC products have been run from written specifications | **DEMO-VERIFIED** | `examples/generalization/results/case_{a,b,c,d,e}.json` with `examples/generalization/PRE_REGISTRATION.md` and `examples/generalization/PRE_REGISTRATION_D_E.md`; `examples/generalization/results/case_{m,p}.json` | "The same pipeline ran seven non-CEC product specifications, with declared coverage limits" | "Fabrivium works for any product" |
| No CEC-120 example data was found silently reused in another product | **DEMO-VERIFIED** | `backend/scripts/generalization_audit.py`; `examples/generalization/results/audit.json` | "The recorded audit found no EXAMPLE_DATA leakage into completed non-CEC concepts" | "No data leakage is possible" |
...
| Simulation is deterministic discrete-event | **IMPLEMENTED** | SimPy; `backend/app/services/simulation.py` | "Deterministic discrete-event simulation" | "Stochastic/Monte-Carlo analysis" — there is none |
...
| The process-family catalog reaches the model dynamically | **TESTED** | `backend/tests/test_prompt_domain_neutrality.py`; composed from `backend/app/services/process_families.py` | "The model is given the canonical twelve families, generated not typed" | — |
| BobProvider is implemented | **IMPLEMENTED** | `backend/app/llm/bob_provider.py` | "An IBM Bob provider is implemented and wired" | "Fabrivium runs on IBM Bob" |
...
| **A live Bob call has been made** | **NOT DONE** | No credential in this environment; `backend/scripts/bob_smoke.py` exists to prove it | "Provider implemented and contract-tested; live validation not run — no credential available" | "Integrated with IBM Bob", "Powered by IBM Bob" |
...
1. **"It works for any product."** It does not. Seven non-CEC product
   specifications have run; two routes are partial; five of twelve families
   have estimation data. Say *"the same pipeline ran seven non-CEC product
   specifications, with declared coverage limits."*
