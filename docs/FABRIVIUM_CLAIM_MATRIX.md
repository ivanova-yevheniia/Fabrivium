...
|---|---|---|---|---|
| CEC-120 goes from 1,435 to 1,900 units/day | **DEMO-VERIFIED** | `docs/CURRENT_COMPETITION_BASELINE.md`, recorded run | "A verified case study: 1,435 → 1,900/day" | "Fabrivium optimises any line to target" |
...
| The figures are reproducible from their inputs | **DEMO-VERIFIED** | `scripts/golden_journey_run.py`, 2026-08-31: 1,435 / 465 / `m-assembly-2` / 1,900 / 3 / 23, from the source PDF through the production API | "Reproducible on demand from the source document" | "Deterministic under any change" |
...
| Claim | Status | Evidence | Allowed wording | Forbidden |
|---|---|---|---|---|
| No production code branches on the product or the demo | **TESTED** | `scripts/fabrivium_coupling_audit.py`, class E = 0, F = 0, exits non-zero on a finding | "No production branch is keyed on the product, the demo or the golden case" | "No demo-specific code exists anywhere" (fixtures and example data do) |
...
| No CEC-120 value reaches another product | **TESTED** | `scripts/generalization_audit.py`: "no leakage or unknown-substitution finding" on every case | "No golden-run figure reaches another product" | "No data leakage is possible" |
...
| Claim | Status | Evidence | Allowed wording | Forbidden |
|---|---|---|---|---|
| A language model cannot reach the simulation core | **TESTED** | `test_deterministic_core_isolation.py` — transitive import graph, 32 core modules | "Enforced by an architecture test, transitively" | "The AI cannot affect results" (it proposes values a person may accept) |
...
| Verification goes stale when inputs move | **TESTED** | Channel model; `test_operation_grouping` pins grouping invalidation | "Changing a simulation input invalidates the verified result" | "Fabrivium always knows what is current" |
...
| Claim | Status | Evidence | Allowed wording | Forbidden |
|---|---|---|---|---|
| Several operations can share one physical resource | **TESTED** | `ConceptOperationGroup`; 27 tests in `test_operation_grouping.py` | "An engineer can declare that one cell performs several operations" | "Fabrivium designs cells" — it records the engineer's decision |
| A grouped architecture simulates | **TESTED** | `test_a_grouped_concept_simulates` | "A grouped concept compiles and simulates" | "Fabrivium optimises cell layout" |
| Cell work content is the sum of its operations | **TESTED** | `test_cell_work_content_is_the_sum_and_never_less` | "Sequential cell work content = the sum" | "Fabrivium models cell concurrency" — only SEQUENTIAL exists |
...
| Claim | Status | Evidence | Allowed wording | Forbidden |
|---|---|---|---|---|
| Production prompts are domain-neutral | **TESTED** | `test_prompt_domain_neutrality.py`, 28 tests over the 6 real prompts | "No production prompt names an industry; verified by test" | "The AI is domain-agnostic" |
...
| BobProvider is contract-tested | **TESTED** | `test_bob_provider.py`, 47 tests against a stubbed transport | "Request construction, parsing, error mapping and key redaction are tested" | "Verified against IBM Bob" |
| **A live Bob call has been made** | **NOT DONE** | No credential in this environment; `scripts/bob_smoke.py` exists to prove it | "Provider implemented and contract-tested; live validation not run — no credential available" | "Integrated with IBM Bob", "Powered by IBM Bob" |
